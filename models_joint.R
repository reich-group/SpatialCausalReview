...
joint <- function(Y,A,ADJ,
                  iters=20000,burn=1000,thin=2,
 ...
 cat("
   model{
     for(i in 1:N){
       ...
       A[i]         ~ dbin(q[i],1)
       muY[i]      <- int1 + beta*A[i] + U[i] + phi*V[i]
       logit(q[i]) <- int2 + V[i]
     }
     ...
     beta  ~ dnorm(0,0.1)
     phi   ~ dnorm(0,0.1)
     taue  ~ dgamma(0.5,0.05)
     tauu  ~ dgamma(0.5,0.05)
     tauv  ~ dgamma(0.5,0.05)
   }
 ", fill=TRUE)
 sink()
 ...
 inits     <- function(){list(taue=1/var(Y), tauu=10,tauv=10,int1=mean(Y),int2=0,beta=0,phi=0,U=0*Y,V=0*Y)}
